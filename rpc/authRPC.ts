...
import { AUTHENTICATION_SERVER_URL } from "@/constants";

const authClient = new JSONRPCClient(async (jsonRPCRequest) => {
    if (AUTHENTICATION_SERVER_URL === undefined) {
        throw new Error("AUTH_SERVER_URL is not defined");
    }

    return axios.post(AUTHENTICATION_SERVER_URL, jsonRPCRequest).then((response) => {
