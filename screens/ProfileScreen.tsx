...
const ProfileScreen: React.FC = () => {
    ...
    return (
        <ScrollView flex={1} bg={colors.darkGray}>
            <VStack px={"20px"} variant={"body"} justifyContent={"space-between"}>
                <VStack>
                    <VStack mt={"10px"} alignItems={"center"} >
                        <ZStack w={scale(100)} h={scale(100)} borderRadius={100} justifyContent={"flex-end"} alignItems={"flex-end"}>
                            {profileImage ?
                                <Pressable w={scale(90)} h={scale(90)} borderRadius={100} onPress={() => onOpenPreviewImage()} _pressed={{ opacity: 0.5 }}>
                        ...
                        <VStack mt={"10px"} ml={"10px"} alignItems={"center"} justifyContent={"center"}>
                            <Heading textTransform={"capitalize"} fontSize={scale(26)} color={"white"}>{MAKE_FULL_NAME_SHORTEN(user?.fullName || "")}</Heading>
                            <Text fontSize={scale(18)} color={colors.lightSkyGray}>{user?.username}</Text>
                        </VStack>
                    </VStack>
                    <VStack px={"10px"} borderRadius={10} w={"100%"} h={"auto"} mt={"30px"} bg={"lightGray"}>
                        <FlatList
